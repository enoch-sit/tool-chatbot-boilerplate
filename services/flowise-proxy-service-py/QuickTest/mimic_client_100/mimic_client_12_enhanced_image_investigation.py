...
def get_token(user):
    ...
    try:
        ...
        if token:
            log_message(f"{Fore.GREEN}✅ Authentication successful for {user['username']}.")
            return token
        else:
            log_message(f"{Fore.RED}❌ Authentication failed for {user['username']}: 'access_token' not in response.")
            return None
    except requests.RequestException as e:
        log_message(f"{Fore.RED}❌ Authentication request failed for {user['username']}: {e}")
        return None
...
def assign_user_to_chatflow(admin_token, chatflow_id, user_email):
    ...
    try:
        ...
        if response.status_code == 200:
            log_message(f"{Fore.GREEN}✅ Successfully assigned user '{user_email}' to chatflow")
            return True
        else:
            log_message(f"{Fore.RED}❌ Failed to assign user: {response.status_code} - {response.text}")
            return False
...
def send_chat_message_with_image(token, username, chatflow_id, question, image_data=None, session_id=None):
    ...
    try:
        with requests.post(url, json=payload, headers=headers, stream=True, timeout=120) as response:
            if response.status_code == 200:
                    ...
                    if chunk:
                        ...
                        for event in events:
                            if event.get("event") == "session_id":
                                ...
                                log_message(f"🔑 Extracted session_id: {extracted_session_id}")
                            elif event.get("event") == "metadata":
                                meta_session_id = event.get("data", {}).get("sessionId")
                                if meta_session_id:
                                    extracted_session_id = meta_session_id
                                    log_message(f"🔑 Extracted session_id from metadata: {extracted_session_id}")
                ...
                log_message(f"{Fore.RED}❌ Stream failed: {response.status_code} - {response.text}")
    ...
    for endpoint in endpoints:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                log_message(f"         ✅ Success - Type: {content_type}, Size: {content_length}")
                
                # If it's an image, try to save it for verification
                if 'image' in content_type:
                    try:
                        filename = f"retrieved_image_{file_id}_{endpoint.split('/')[-1]}.png"
...
def main():
    ...
    if not assign_user_to_chatflow(admin_token, target_chatflow_id, REGULAR_USER["email"]):
    ...
    if not session_id:
        log_message(f"{Fore.RED}❌ Critical error: Could not obtain session_id. Aborting test.")
        sys.exit(1)
    ...
    assistant_reply_2, _ = send_chat_message_with_image(
        user_token, REGULAR_USER["username"], target_chatflow_id, follow_up_question, None, session_id
    )
    ...
    log_message(f"   > Session ID: {session_id}")
    log_message(f"   > Image upload: {'✅ Success' if assistant_reply else '❌ Failed'}")
    log_message(f"   > Follow-up response: {'✅ Success' if assistant_reply_2 else '❌ Failed'}")
