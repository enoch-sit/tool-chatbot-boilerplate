...
class AccountingService:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Accounting service error (check_user_credits for {user_id}): {response.status_code} - {response.text}")
                    return None
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success"):
                        ...
                        print(f"Credit deduction failed for {user_id}: {data.get('message')}")
                        return False
                else:
                    print(f"Credit deduction error for {user_id}: {response.status_code} - {response.text}")
                    return False
    ...
    async def get_operation_cost(self, model_id: str, tokens: int) -> int: # Signature changed
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Cost lookup error for model {model_id}: {response.status_code} - {response.text}")
    ...
    async def log_transaction(
        ...
        try:
            ...
            final_metadata.setdefault("user_id_source", user_id) # Original user_id if needed for cross-referencing
            final_metadata.setdefault("original_operation_details", operation_name) # e.g. chatflow_id
