...
echo -e "${CYAN}==========================================${NC}"
echo -e "${CYAN}Chat Service Docker Rebuild Script${NC}"
echo -e "${CYAN}==========================================${NC}"
...
if [ $accounting_running -ne 0 ]; then
    echo -e "\n${YELLOW}Accounting service is not running but is recommended.${NC}"
    read -p "Do you want to start Accounting service? (y/n) " start_accounting
    
    if [ "$start_accounting" == "y" ]; then
...
echo -e "\n${YELLOW}Stopping existing chat service containers...${NC}"
docker-compose down

# Remove old images (optional - uncomment if needed)
# echo -e "\n${YELLOW}Removing old images...${NC}"
# docker rmi -f $(docker images -q 'chat-service_chat-service' 2>/dev/null) || true

# Rebuild and start the containers
echo -e "\n${YELLOW}Rebuilding and starting chat service containers...${NC}"
...
if [ $chat_running -eq 0 ]; then
    ...
    echo -e "\n${GREEN}Chat service was rebuilt and started successfully at http://localhost:3002${NC}"
else
    echo -e "${RED}Chat: Not running ❌${NC}"
    echo -e "\n${RED}Failed to rebuild Chat service. Please check the logs for errors.${NC}"
fi
