    ...
    switch (event.event) {
      case "agentFlowEvent":
        if (typeof event.data === 'object' && event.data !== null) {
          return event.data.status || t('agentFlow.event');
        }
        return t('agentFlow.event');
      case "nextAgentFlow":
        return t('agentFlow.nextFlow');
      case "agentFlowExecutedData":
        return t('agentFlow.executed');
      case "calledTools":
        return t('agentFlow.toolsCalled');
      case "start":
        return t('agentFlow.start');
      case "end":
        return t('agentFlow.end');
      case "session_id":
        return t('agentFlow.sessionId');
      default:
        return t('agentFlow.event');
    }
  };
    ...
    <Box sx={{ mb: 2 }}>
      ...
      <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <ModalDialog sx={{ maxWidth: 600, width: '90%' }}>
          <ModalClose>
            <Typography level="h2">{t('agentFlow.eventDetails')}</Typography>
          </ModalClose>
          <Divider sx={{ my: 2 }} />
          <CardContent>
            {selectedEvent && (
              <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
                {selectedEvent.event}
                {selectedEvent && 'data' in selectedEvent && (
                  <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
