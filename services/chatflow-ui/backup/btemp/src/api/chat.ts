 ...
 * @param onStreamEnd A callback function invoked when the stream ends.
...
export const streamChatAndStore = async (
  ...
  onStreamEnd: () => void, // Add this line
    ...
    if (response.status === 401) {
      ...
      try {
        ...
        if (newTokens?.accessToken) {
          ...
          await processStreamResponse(retryResponse, onStreamEvent, onStreamEnd, onError);
          return;
        }
      } catch (refreshError) {
  ...
  await processStreamResponse(response, onStreamEvent, onStreamEnd, onError);
};
...
const processStreamResponse = async (
  ...
  try {
    while (true) {
      ...
      streamParser.processChunk(decoder.decode(value, { stream: true }));
    }
  } catch (error) {
